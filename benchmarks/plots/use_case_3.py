...
OUTDIR = None
THREADS = 4
SETUPS = {"H_base": {}, "H_I_1": {}, "H_I_2": {}}
TRACE_NAMES = ["cluster18", "cluster53", "cluster40", "cluster19"]
THREAD_NAMES = [f"I$_{i+1}$" for i in range(THREADS)]
QOS = {setup: [0 for _ in range(THREADS)] for setup in SETUPS}
...
def plot(
    ...
    for i, (setup, metrics) in enumerate(throughput_per_setup.items()):
        for j, values in enumerate(metrics):
            ...
            if QOS[setup][j] > 0:
                axs[j, i].axhline(
                    y=QOS[setup][j], color="red", linestyle=":", linewidth=1
                )
    ...
    by_label = {
        ...
        "QoS": plt.Line2D([0], [0], color="red", lw=1, linestyle=":"),
    }
    fig.legend(
        by_label.values(),
        by_label.keys(),
...
def main():
    ...
    for setup in SETUPS:
        with open(results_dir / setup / "setup.json", "r") as f:
            ...
            for i in range(THREADS):
                if qos_value := SETUPS[setup].get(f"holpaca.pool.qos.{i}", None):
                    QOS[setup][i] = qos_value
